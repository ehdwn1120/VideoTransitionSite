...
- AdSense 계정에서 사이트 소유권 확인 방법과 게시자 ID를 받아 실제 설정을 연결해야 합니다. 현재 저장소에는 사용자가 추가한 AdSense 소유확인 태그와 게시자 ID가 있습니다. 승인 상태는 AdSense 콘솔에서 별도 확인해야 합니다.
- ads.txt가 필요한 설정에서는 해당 계정에서 제공하는 정확한 레코드를 사용하세요. 현재 public/ads.txt에는 사용자가 등록한 게시자 ID가 있습니다.
