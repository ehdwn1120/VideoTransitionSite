...
export class ImageEngine {
  ...
  check() { if (this.cancelled) throw new Error('변환을 중지했습니다.'); }
  async convert(file, options) {
    const format = imageFormats[options.format];
    if (!format || !['high','balanced','small'].includes(options.quality)) throw new Error('출력 설정을 확인해 주세요.');
    const bytes = new Uint8Array(await file.arrayBuffer());
    this.check();
    const signature = String.fromCharCode(...bytes.subarray(0, 12));
    if (!(bytes[0] === 255 && bytes[1] === 216 && bytes[2] === 255) &&
        !signature.startsWith('\x89PNG\r\n\x1a\n') &&
        !(signature.startsWith('RIFF') && signature.slice(8) === 'WEBP')) throw new Error('JPG·PNG·WebP 이미지 내용이 아닙니다. 확장자와 파일 손상을 확인해 주세요.');
    if (isAnimated(bytes)) throw new Error('움직이는 PNG·WebP는 지원하지 않습니다. 정지 이미지를 선택해 주세요.');
    let bitmap, canvas;
    try {
      try { bitmap = await createImageBitmap(file); }
      catch { throw new Error('이미지를 읽을 수 없습니다. 손상된 파일이거나 이 브라우저에서 지원하지 않는 이미지입니다.'); }
      this.check();
      if (bitmap.width * bitmap.height > 40_000_000) throw new Error('이미지는 4천만 픽셀 이하로 선택해 주세요.');
      ...
      if (!context) throw new Error('이 브라우저에서는 이미지 변환을 사용할 수 없습니다.');
      ...
      this.check();
      if (!blob || blob.type !== format.mime) throw new Error('이 브라우저는 선택한 이미지 포맷의 저장을 지원하지 않습니다. 다른 포맷을 선택해 주세요.');
