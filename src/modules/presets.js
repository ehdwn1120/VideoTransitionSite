...
export function describeOutput({format, resolution, quality, fps, duration}) {
  const qualityLabel = { high: t('높은 품질'), balanced: t('균형 잡힌 품질'), small: t('작은 파일') }[quality];
  const height = resolution === 'original' ? t('원본 해상도') : t('높이 최대 {height}px', {height:resolution});
  if (format === 'mp3') return t('MP3 · {bitrate} · 오디오만 저장', {bitrate:AUDIO_BITRATES.mp3[quality].replace('k','kbps')});
  if (format === 'gif') return t('GIF · 시작부터 {duration}초 · {fps}fps · {height} · 소리 없음', {duration,fps,height});
  return `MP4 · ${height} · ${qualityLabel} · AAC ${AUDIO_BITRATES.mp4.replace('k', 'kbps')}`;
}
export const presets = {
  mp4: { label: t('MP4 비디오'), extension: 'mp4', mime: 'video/mp4' },
  gif: { label: t('GIF 애니메이션'), extension: 'gif', mime: 'image/gif' },
  mp3: { label: t('MP3 오디오'), extension: 'mp3', mime: 'audio/mpeg' },
};
export function buildArgs(input, options) {
  const { format, resolution, quality, fps, duration } = options;
  if (!presets[format]) throw new Error(t('지원하지 않는 출력 포맷입니다.'));
  if (!['original', '1080', '720', '480'].includes(resolution)) throw new Error(t('해상도를 확인해 주세요.'));
  if (!['high', 'balanced', 'small'].includes(quality)) throw new Error(t('품질을 확인해 주세요.'));
  ...
  if (format === 'gif') {
    if (![10, 15, 24].includes(Number(fps)) || !Number.isFinite(Number(duration)) || duration < 1 || duration > 30) throw new Error(t('GIF 길이는 1~30초로 설정해 주세요.'));
...
export function validateFile(file) {
  if (detectMedia(file) !== 'video') throw new Error(t('영상 파일을 선택해 주세요.'));
}
