...
function setBusy(value) {
  ...
  $('cancel').hidden = !value;
  $('engine-status').textContent = value ? t('처리 중') : t('대기 중');
}
...
function resetSelection() {
  ...
  $('file-title').textContent = t('이미지나 영상을 여기에 드롭하세요');
  $('file-meta').textContent = t('또는 클릭하여 파일 선택');
...
function selectFile(next) {
  ...
  $('file-meta').textContent = t('{kind} · {size} · 클릭하여 파일 변경', {kind: mediaKind === 'image' ? t('이미지') : t('영상'), size:size(file.size)});
  selectionMessage(t('선택 완료: {name} · {size}. 아래에서 변환 실행을 눌러 주세요.', {name:file.name,size:size(file.size)}));
  setBusy(false);
  ...
  else selectionMessage(t('선택 완료: {name} · {size}. 원본 미리보기 없이 변환할 수 있습니다. 아래에서 변환 실행을 눌러 주세요.', {name:file.name,size:size(file.size)}));
  if (mediaKind === 'video' && file.size > LARGE_VIDEO_BYTES) selectionMessage(t('선택 완료: {name} · {size}. 큰 영상은 PC 사용을 권장합니다. 모바일에서는 메모리 부족으로 실패하거나 탭이 종료될 수 있습니다. 원본 미리보기는 생략합니다.', {name:file.name,size:size(file.size)}));
  $('source-preview').hidden = false;
  $('progress').hidden = true;
  $('logs').textContent = '';
  status(t('파일 준비 완료. 출력 설정을 확인하고 변환을 실행하세요.'));
  setBusy(false);
}
$('file').addEventListener('change', e => { if (e.target.files[0]) selectFile(e.target.files[0]); else selectionMessage(t('파일을 받지 못했습니다. 기기에 저장한 파일을 다시 선택해 주세요.'), true); });
$('file').addEventListener('click', () => { $('file').value = ''; });
$('file').addEventListener('cancel', () => { if (!file) selectionMessage(t('파일 선택이 취소되었습니다. 클라우드 파일은 기기에 내려받은 뒤 다시 선택해 주세요.')); });
...
$('dropzone').addEventListener('drop', e => { if (e.dataTransfer.files.length > 1) { status(t('한 번에 한 파일씩 선택해 주세요.')); return; } if(e.dataTransfer.files[0]) selectFile(e.dataTransfer.files[0]); });
window.addEventListener('dragover', e => e.preventDefault());
window.addEventListener('drop', e => e.preventDefault());
$('remove').addEventListener('click', () => {
  resetSelection(); selectionMessage(t('파일을 제거했습니다. 새 파일을 선택해 주세요.'));
  status(t('변환할 이미지나 영상을 선택하세요.'));
});
function updateOptions() {
  ...
  $('engine-hint').textContent = mediaKind === 'image' ? t('브라우저에서 이미지 처리 · 별도 엔진 다운로드 없음') : t('첫 영상 변환 시 약 31MB의 엔진을 불러옵니다.');
  $('engine-label').textContent = mediaKind === 'image' ? t('변환: 브라우저 이미지 처리') : t('변환: FFmpeg');
...
$('cancel').addEventListener('click', () => { run++; engine.cancel(); setBusy(false); $('progress').hidden = true; status(t('변환을 중지했습니다. 다시 실행할 수 있습니다.')); });
$('convert-form').addEventListener('submit', async e => {
  ...
  status(mediaKind === 'image' ? t('이미지를 변환하는 중…') : t('변환 엔진을 불러오는 중… 첫 실행에는 시간이 걸릴 수 있습니다.'));
  try {
    let data, dimensions;
    if (mediaKind === 'image') {
    ...
    data = await taskEngine.convert(file, args, message => {
      ...
      $('progress').value = percent; status(t('변환 중… {percent}% · 탭을 열어 두세요.', {percent}));
    }, (part,total) => { if(current === run) status(t('변환 엔진 준비 중… {part}/{total} 파일 로드 완료', {part,total})); });
    }
    if (current !== run) return;
