...
for (const page of pages) {
  ...
  if(page === 'contact') {
    const block = `${operator ? `<p>운영자: ${escape(operator)}</p>` : ''}${contact ? `<p>이메일: <a class="contact-link" href="mailto:${escape(contact)}">${escape(contact)}</a></p><p>링크를 누르면 기기의 메일 앱이 열립니다. 직접 전송한 이메일만 접수되며, 이 웹사이트가 파일을 자동으로 전송하지 않습니다.</p>` : '<p>공개 문의 이메일은 아직 등록되지 않았습니다. 현재 이 페이지에서 문의를 전송하거나 접수할 수는 없습니다.</p>'}`;
...
await writeFile('dist/sitemap.xml', `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${indexable ? pages.map(page => `  <url><loc>${escape(origin)}/${page === 'index' ? '' : page}</loc></url>`).join('\n') : ''}\n</urlset>\n`);
